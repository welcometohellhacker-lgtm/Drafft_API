...
class JobOrchestratorService:
    ...
    def process(self, job: Job, render_selected_immediately: bool = False, regenerate_transcript: bool = False) -> Job:
        ...
        self.db.query(Asset).filter(Asset.job_id == job.id, Asset.asset_type.in_(["visual_plan", "caption_plan", "clip_candidate_json", "broll_plan", "rendered_clip", "thumbnail"])).delete(synchronize_session=False)
